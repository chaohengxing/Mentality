...
public class TeacherDatumActivity extends ParentWithNaviActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_teacher_datum);
    ...
    protected void initView() {
        super.initView();
        if (TextUtils.isEmpty(UserNetwork.getInstance().getCurrentUser().getTeacherIcon())) {
            ivIcon.setImageResource(R.drawable.default_pic);
        } else {
            PicassoUtils.loadImage(UserNetwork.getInstance().getCurrentUser().getTeacherIcon(), ivIcon);
        }

        tvNickname.setText(UserNetwork.getInstance().getCurrentUser().getTeacherNickName());
        tvSex.setText(UserNetwork.getInstance().getCurrentUser().getSex());
        tvIntro.setText(UserNetwork.getInstance().getCurrentUser().getTeacherIntro());
        ...
        switch (view.getId()) {
            case R.id.iv_icon:
                if (canEdit) {
                    ...
                    images.add(UserNetwork.getInstance().getCurrentUser().getTeacherIcon());
            ...
            case R.id.btn_edit:
                ...
                if (canEdit) {
                    ...
                    if (TextUtils.isEmpty(tvNickname.getText().toString().trim()) ||
                            ...
                            TextUtils.isEmpty(tvIntro.getText().toString().trim())) {
                        toast("信息不能有空，请完整填写!");
                        return;
                    }
                    commint();
    ...
    private void commintStudentInfo(String imageUrl) {
        ...
        UserNetwork.getInstance().updateTeacherInfo(
                UserNetwork.getInstance().getCurrentUser().getId(),
                TextUtils.isEmpty(imageUrl) ? UserNetwork.getInstance().getCurrentUser().getTeacherIcon() : imageUrl,
                tvNickname.getText().toString(),
                tvSex.getText().toString(),
                tvIntro.getText().toString(),
                new UserNetwork.updateTeacherInfoCallBack() {

                    @Override
                    public void response(int responseCode) {

                        if (UserNetwork.UPDATE_TEACHERINFO_YES == responseCode) {
                            toast("提交成功!");
                            finish();
                        } else if (UserNetwork.UPDATE_TEACHERINFO_NO == responseCode) {
                            toast("提交失败!");
                        }
                    }
                }
